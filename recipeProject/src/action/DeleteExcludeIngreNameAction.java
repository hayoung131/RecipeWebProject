...
import svc.DeleteExcludeIngreNameService;

public class DeleteExcludeIngreNameAction implements Action2 {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String [] ingre_arr = request.getParameterValues("idx[]");
		int chNum = Integer.parseInt(request.getParameter("checkNum"));
		System.out.println("몇개 삭제함? "+ chNum);
		
		//재료들 선택안하고 삭제버튼 눌렀을때 아무일도 안일어나게 하는 부분.
		if(chNum==0) {
			String addBotButton = 
		...
		System.out.println("제외할 재료들"+Arrays.toString(ingre_arr));
		
		DeleteExcludeIngreNameService deleteExcludeIngreNameService
		= new DeleteExcludeIngreNameService();
		
		String checkSuccess = deleteExcludeIngreNameService.deleteExIngre(user_id,ingre_arr);
		if(checkSuccess!=null) 
			System.out.println("제외할 재료 삭제 성공!!!");

		String addBotButton = "<div class='question1' id = 'exclude_div' style='margin: 10px; margin-left: 50px;'>"+
				"<input id = 'excludeIngre' type='text' placeholder='제외할 재료를 추가하려면 여기에 입력해주세요.' style='margin:10px; width:200px;'>"+
				  "<button onclick='submitFunction(0)' class='btn btn-secondary btn-sm' role='button' aria-pressed='true'"+
				  "style='text-align: right;margin:2px;padding:4px 10px 4px 10px;' type='submit' name='exceptBtn'>추가</button>"+
				  "<button onclick='submitFunction(1)' class='btn btn-secondary btn-sm' role='button' aria-pressed='true'"+
				  "style='text-align: right;margin:2px;padding:4px 10px 4px 10px;' type='submit' name='exceptBtn'>삭제</button>"+
