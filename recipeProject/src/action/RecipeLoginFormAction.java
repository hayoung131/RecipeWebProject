...
public class RecipeLoginFormAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//1. 정보를 request나 영역객체로부터 받아올게 있다면 받아와 data bean 객체에 넣기
		...
		boolean loginSuccess=recipeLoginFormService.login(/*사용자가 입력했던 정보들이 든 데이터빈*/);
		ActionForward forward = null;//메소드에서 최종적으로 반환할 포워드값을 저장할 놈
		if(loginSuccess){
			//성공시 레시피홈페이지 메인화면 보여주기
			forward = new ActionForward();
			forward.setRedirect(true);//포워딩 방식을 리다이랙트로하겟다.
			forward.setUrl("메인화면.bo");
		}
		else{
